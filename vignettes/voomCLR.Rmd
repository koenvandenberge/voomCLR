...
The `voomCLR` workflow works much as a usual limma-voom workflow, with two changes:
 ...
 - The `voom` function is replaced by a `voomCLR` function. This function transforms the counts using a centered-log-ratio (CLR) transformation, and estimates observation-level heteroscedasticity weights. The latter can be estimated empirically or analytically, as specificed using the `varCalc` argument. 
...
design <- model.matrix(~group)
v <- voomCLR(counts = Y,
             design = design) # span for the lowess smoother
fit <- lmFit(v, design)
...
ttBoot1 <- topTableBC(fit, coef=2, n=Inf, 
                      bootstrap="parametric",
                      voomWeights = v$weights)


## using contrast matrix: also provide this matrix, specifying the relevant column.
contFit <- contrasts.fit(fit, contrasts=cm)
contFit <- eBayes(contFit)
ttBoot2 <- topTableBC(contFit, coef=1, n=Inf, 
                 bootstrap="parametric",
                 voomWeights = v$weights,
